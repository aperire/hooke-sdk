...
class Libertee {
    ...
    */
    ...
    getMediaOwnershipMap = async(address, index) => {
        const ownerMedia = await this.contract.mediaOwnershipMap(address, index);
        return ownerMedia;
    }
    
    getProfileMap = async(address) => {
        const profile = await this.contract.profileMap(address);
        return profile;
    }
    
    getProfileNameMap = async(name) => {
        const profile = await this.contract.profileNameMap(name);
        return profile;
    }
    
    getHashTagMap = async(hashTag, index) => {
        const media = await this.contract.hashTagMap(hashTag, index);
        return media
    }
